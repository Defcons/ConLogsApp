...
--
-- Loaded after ConLogs.lua (TOC order) so SlashCmdList["CONLOGS"]
...
local function FindPlayer(name)
    ...
    if not ConLogsDB or not ConLogsDB.players then return nil end
...
local cacheTip = CreateFrame("GameTooltip", "ConLogsCacheTip", UIParent, "GameTooltipTemplate")
...
local function MakeSlotButton(parent, slotID)
    local b = CreateFrame("Button", "ConLogsSlotBtn" .. slotID, parent)
...
-- ConLogs.DeletePlayer) also opens our 24h mesh-dedup gate immediately.
StaticPopupDialogs["CONLOGS_CONFIRM_DELETE"] = {
    ...
    OnAccept = function(self)
        ...
        if _G.ConLogs and _G.ConLogs.DeletePlayer then
            _G.ConLogs.DeletePlayer(guid)
        end
...
StaticPopupDialogs["CONLOGS_CONFIRM_SYNC"] = {
    ...
    OnAccept = function(self)
        ...
        if name and SlashCmdList and SlashCmdList["CONLOGS"] then
            SlashCmdList["CONLOGS"]("syncfrom " .. name)
        end
    end,
    timeout = 0,
    whileDead = true,
...
local function BuildInspectFrame()
    local f = CreateFrame("Frame", "ConLogsInspectFrame", UIParent)
    ...
    delete:SetScript("OnClick", function()
        ...
        local dialog = StaticPopup_Show("CONLOGS_CONFIRM_DELETE", p.name or "?")
    ...
    talents:SetScript("OnClick", function()
        if not f.activePlayer then return end
        if _G.ConLogs_OpenTalentsFor then
            _G.ConLogs_OpenTalentsFor(f.activePlayer, f.activeGroup)
        end
    end)
    ...
    stats:SetScript("OnClick", function()
        if not f.activePlayer then return end
        if _G.ConLogs_OpenStatsFor then
            _G.ConLogs_OpenStatsFor(f.activePlayer, f.activeGroup)
        end
    end)
    f.statsBtn = stats
    ...
    tinsert(UISpecialFrames, "ConLogsInspectFrame")
    return f
end
...
-- inspect frame when shown. Reads metadata from ConLogsTalentTreeDB
...
local function BuildTalentFrame()
    ...
    local t = CreateFrame("Frame", "ConLogsTalentFrame", UIParent)
    ...
    -- to the inspect frame's RIGHT edge in ConLogs_OpenTalentsFor, so
    ...
    -- "No metadata yet" placeholder, shown when ConLogsTalentTreeDB doesn't
    ...
    function t.RenderTab(tabIdx)
        ...
        local meta  = ConLogsTalentTreeDB and ConLogsTalentTreeDB[cls]
            and ConLogsTalentTreeDB[cls].tabs and ConLogsTalentTreeDB[cls].tabs[tabIdx]
    ...
    tinsert(UISpecialFrames, "ConLogsTalentFrame") -- Esc closes
    return t
end
...
function _G.ConLogs_OpenTalentsFor(player, group)
...
-- Aggregates item stats from ConLogsItemCacheDB across the active set's gear,
...
local function AggregateItemStats(gear)
    ...
    local cache = ConLogsItemCacheDB or {}
    for slot = 1, 19 do
...
local function BuildStatsFrame()
    local f = CreateFrame("Frame", "ConLogsStatsFrame", UIParent)
    ...
    tinsert(UISpecialFrames, "ConLogsStatsFrame")
    return f
end

-- Public opener — called from the inspect frame's "Stats" button.
function _G.ConLogs_OpenStatsFor(player, group)
...
local function ShowInspect(player, group)
    ...
    if talentFrame and talentFrame:IsShown() and _G.ConLogs_OpenTalentsFor then
...
local function BuildBrowser()
    local f = CreateFrame("Frame", "ConLogsBrowserFrame", UIParent)
    ...
    local search = CreateFrame("EditBox", "ConLogsBrowserSearch", f, "InputBoxTemplate")
    ...
    local classFilterMenu = CreateFrame("Frame", "ConLogsClassFilterMenu", f, "UIDropDownMenuTemplate")

    f.classFilterBtn = CreateFrame("Button", "ConLogsClassFilterBtn", f, "UIPanelButtonTemplate")
    ...
    -- Scroll frame
    local scroll = CreateFrame("ScrollFrame", "ConLogsBrowserScroll", f, "FauxScrollFrameTemplate")
        ...
        row:SetScript("OnClick", function(self)
            if f.viewMode == "scanners" then
                if self.activeSync then
                    print("|cffffaa44ConLogs|r: already syncing from this peer — wait for it to finish.")
                    return
                end
                if self.reachable == false then
                    print(string.format(
                        "|cffffaa44ConLogs|r: %s is offline or not in your guild/group right now — can't sync.",
                        self.scannerName or "peer"))
                    return
                end
                if self.rowGreyed then
                    print(string.format("|cffffaa44ConLogs|r: at the %d-sync limit. Wait for one to finish first.",
                        (_G.ConLogs and _G.ConLogs.SyncMaxConcurrent) or 3))
                    return
                end
                -- Clicked a scanner-leaderboard row → pop confirm + trigger sync
                if self.scannerName and self.scannerName ~= "" then
                    local dlg = StaticPopup_Show("CONLOGS_CONFIRM_SYNC", self.scannerName)
    ...
    -- Default enabled on first login; state persisted in ConLogsDB.config.
    -- v0.38: label is parented to the button itself so Hide() hides both.
    f.acceptSyncBtn = CreateFrame("CheckButton", "ConLogsAcceptSyncBtn", f, "UICheckButtonTemplate")
    ...
    f.acceptSyncBtn:SetScript("OnClick", function(self)
        ...
        ConLogsDB.config.acceptSync = self:GetChecked() and true or false
        if ConLogsDB.config.acceptSync then
            print("|cffffaa44ConLogs|r: sync-response |cff00ff66ON|r")
        else
            print("|cffffaa44ConLogs|r: sync-response |cffff6666OFF|r (will refuse incoming SYNCREQ)")
        end
    end)
    ...
    f.refreshPeersBtn = CreateFrame("Button", "ConLogsRefreshPeersBtn", f, "UIPanelButtonTemplate")
    ...
    f.refreshPeersBtn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Asks every guildmate running ConLogs to send their current identity and DB size.", 1, 1, 1, true)
        GameTooltip:AddLine(" ")
    ...
    f.refreshPeersBtn:SetScript("OnClick", function(self)
        if not (_G.ConLogs and _G.ConLogs.RequestPeerRefresh) then return end
        local ok, info, extra = _G.ConLogs.RequestPeerRefresh()
        if ok then
    ...
    local function AggregateScanners()
        ...
        local peerInfo = (ConLogsDB and ConLogsDB.peerInfo) or {}
        ...
        -- it inline here from ConLogsDB.players. Only touch reportedDB
        ...
        local myIdentity = (_G.ConLogs and _G.ConLogs.MyIdentity)
            and _G.ConLogs.MyIdentity() or UnitName("player")
        if myIdentity and myIdentity ~= "" and stats[myIdentity] then
            local myDBSize = 0
            if ConLogsDB and ConLogsDB.players then
                for _ in pairs(ConLogsDB.players) do myDBSize = myDBSize + 1 end
            end
    ...
    local function UpdatePlayersMode()
        ...
        local total = 0
        if ConLogsDB and ConLogsDB.players then
            for _ in pairs(ConLogsDB.players) do total = total + 1 end
        end
        if total == 0 then
            f.emptyHint:Show()
    ...
    local function UpdateScannersMode()
        ...
        -- v0.37: read sync state from ConLogs namespace.
        local isActive = (_G.ConLogs and _G.ConLogs.IsPeerSyncActive) or function() return false end
        local syncEndsAt = (_G.ConLogs and _G.ConLogs.SyncEndTimeFor) or function() return nil end
        local activeCount = (_G.ConLogs and _G.ConLogs.ActiveSyncCount and _G.ConLogs.ActiveSyncCount()) or 0
        local maxConcurrent = (_G.ConLogs and _G.ConLogs.SyncMaxConcurrent) or 3
    ...
    tinsert(UISpecialFrames, "ConLogsBrowserFrame")
    ...
    _G.ConLogs.OnPlayerChanged = function()
...
SaveFramePosition = function(frame)
    ...
    ConLogsDB = ConLogsDB or {}
    ConLogsDB.config = ConLogsDB.config or {}
    ConLogsDB.config.framePos = { p, rp or p, x or 0, y or 0 }
end

ApplySavedPosition = function(frame)
    if not frame then return end
    local fp = ConLogsDB and ConLogsDB.config and ConLogsDB.config.framePos
...
BackToBrowser = function()
    ...
    if _G.ConLogsTalentFrame and _G.ConLogsTalentFrame:IsShown() then
        _G.ConLogsTalentFrame:Hide()
    end
    browserFrame:Show()
end
...
local function BuildMinimapButton()
    ...
    local b = CreateFrame("Button", "ConLogsMinimapButton", Minimap)
    ...
    local minimapMenu = CreateFrame("Frame", "ConLogsMinimapMenu", UIParent, "UIDropDownMenuTemplate")
    local function InitMinimapMenu(self, level)
        ...
        add("Wipe DB",      function() SlashCmdList["CONLOGS"]("wipe") end)
    ...
    b:SetScript("OnEnter", function(self)
        ...
        local _ver = (_G.ConLogs and _G.ConLogs.VERSION)
            or (GetAddOnMetadata and GetAddOnMetadata("ConLogs-CoA", "Version")) or nil
    ...
    b:SetScript("OnDragStart", function(self)
        self:SetScript("OnUpdate", function()
            ...
            ConLogsDB = ConLogsDB or {}
            ConLogsDB.config = ConLogsDB.config or {}
            ConLogsDB.config.minimapAngle = angle
...
mmInit:SetScript("OnEvent", function()
    ...
    local angle = (ConLogsDB and ConLogsDB.config and ConLogsDB.config.minimapAngle)
...
SlashCmdList["CONLOGS"] = function(msg)
    ...
    if lcmd == "show" or lcmd == "inspect" then
        ...
        if not player then
            print(string.format("|cffffaa44ConLogs|r: no stored snapshot for '%s'", name))
            return
        end
