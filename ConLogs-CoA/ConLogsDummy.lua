-- ============================================================================
-- ConLogsDummy.lua — Dummy parse validation + combat log gating
-- ============================================================================
...
-- Realm gating. Override list lives in ConLogsDB.skipAoERealms when
-- the user needs to add an exact realm string the defaults miss; the
-- login chat message in ConLogs.lua surfaces the resolved realm so
...
local function ShouldSkipAoEOnThisRealm()
    ...
    local override = ConLogsDB and ConLogsDB.skipAoERealms
...
--
-- See `ConLogsValidateButton` creation in BuildFrame for the actual
-- secure-button wiring.

local function _DebugPrint(msg)
    if ConLogsDebug then
        print("|cffffaa44ConLogs|r |cff888888[dummy-debug]|r " .. msg)
    end
end
...
local function PrintVerdict()
    ...
    if IsCleanVerdict() then
        print("|cffffaa44ConLogs|r: |cff66ff66CLEAN dummy parse|r - log is valid for upload.")
        if logFilename then
            print("  |cffaaaaaa-|r upload this file: |cffffd200" .. logFilename .. "|r")
        end
        return
    end
    print("|cffffaa44ConLogs|r: |cffff6666INVALID dummy parse|r - log will be rejected. Reasons:")
    local count = 0
...
local function SaveFightRecord()
    ...
    ConLogsDB.dummyFights = ConLogsDB.dummyFights or {}
    ...
    -- never lands in ConLogsDB.dummyFights, so it can't be exported
    ...
    if extraDummyCount > 0 and ShouldSkipAoEOnThisRealm() then
        print("|cffffaa44ConLogs|r: |cffff5555AoE dummy session ignored|r — "
            .. "hit " .. (extraDummyCount + 1) .. " dummies (e.g. "
            .. (aoeExtraDummyName or "?") .. "). "
            .. "epoglogs accepts single-target only.")
        StaticPopup_Show("CONLOGS_AOE_DUMMY_REJECTED")
        return
    end
    ...
    table.insert(ConLogsDB.dummyFights, {
        endTime        = floor(time()),
        ...
        addonVersion   = GetAddOnMetadata and GetAddOnMetadata("ConLogs-CoA", "Version") or "?",
    ...
    while #ConLogsDB.dummyFights > MAX_FIGHT_HISTORY do
        table.remove(ConLogsDB.dummyFights, 1)
    end
end
...
local function FinishFight(reason)
    ...
    if testMode then
        if markerVerified then
            print("|cffffaa44ConLogs|r [testvalidate]: |cff66ff66MARKER VERIFIED|r — the secure-button mechanism works on this client.")
            if logFilename then
...
local function OnEnterCombat()
    ...
    if state == "idle" and IsDummyTargeted() and IsCity() then
        local autoLog = ConLogsDB and ConLogsDB.config
            and ConLogsDB.config.dummyAutoLog
        if autoLog then
            DoStartLogging()
        else
            DoStartPractice()
...
local function BuildFrame()
    ...
    f.validateBtn = CreateFrame("Button", "ConLogsValidateButton",
    ...
    f.autoLogCheck = CreateFrame("CheckButton", "ConLogsDummyAutoLog", f, "UICheckButtonTemplate")
    ...
    f.autoLogCheck:SetScript("OnClick", function(self)
        ...
        ConLogsDB.config.dummyAutoLog = self:GetChecked() and true or false
    end)
    ...
    function f.UpdateUI()
        ...
        local autoLog = (ConLogsDB and ConLogsDB.config and ConLogsDB.config.dummyAutoLog) or false
...
_testStopFrame:SetScript("OnUpdate", function(self, e)
    ...
    if _testStopElapsed >= 0.15 then
        ...
        if SpellStopCasting then
            SpellStopCasting()
            print("|cffffaa44ConLogs|r [testcast] SpellStopCasting() called")
        end
    end
end)
...
_testWatchFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if spellName == _testWatchSpellName then
        if not _testWatchSeen then
            print(string.format("|cffffaa44ConLogs|r [testcast] |cff66ff66LANDED|r in CLEU after %.2fs: %s spellID=%s",
...
_G.ConLogsDummy_TestCast = function(spellName, stopAfter)
    spellName = (spellName and spellName ~= "") and spellName or "Hearthstone"
    print(string.format("|cffffaa44ConLogs|r [testcast] starting test for '%s'", spellName))
    print(string.format("|cffffaa44ConLogs|r [testcast]   InCombatLockdown=%s, GetSpellInfo='%s'",
    ...
    if not CastSpellByName then
        print("|cffffaa44ConLogs|r [testcast] |cffff6666CastSpellByName is nil!|r")
        return
    end
    CastSpellByName(spellName)
    print(string.format("|cffffaa44ConLogs|r [testcast]   CastSpellByName('%s') returned, no error",
        spellName))
...
-- secure validate button. Users go to Keybindings UI -> ConLogs
-- Dummy Parse -> Validate Parse to assign a hotkey.
_G["BINDING_HEADER_CONLOGS_DUMMY"] = "ConLogs Dummy Parse"
_G["BINDING_NAME_CLICK ConLogsValidateButton:LeftButton"] = "Validate Parse"

_G.ConLogsDummy_Toggle = function()
...
_G.ConLogsDummy_TestValidate = function()
    if state == "logging" or state == "stopping" then
        print("|cffffaa44ConLogs|r [testvalidate]: a real parse is in progress — refusing to start test mode. Click Reset/Stop first.")
        return
    end
    if not frame then frame = BuildFrame() end
    ...
    print("|cffffaa44ConLogs|r [testvalidate]: |cffffd200test mode active.|r")
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_LOGIN" then
        ConLogsDB = ConLogsDB or {}
        ConLogsDB.config = ConLogsDB.config or {}
        if ConLogsDB.config.dummyAutoLog == nil then
            ConLogsDB.config.dummyAutoLog = false
        end
        ConLogsDB.dummyFights = ConLogsDB.dummyFights or {}
...
SlashCmdList["CONLOGSTEST"] = function(msg)
    ...
    if _G.ConLogsDummy_TestCast then
        _G.ConLogsDummy_TestCast(spellName, stopAfter)
    else
        print("|cffffaa44ConLogs|r [testcast] module not loaded")
    end
end
