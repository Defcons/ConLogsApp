-- ============================================================================
-- ConLogsDungeon.lua — Dungeon speedrun tracker frame
-- ============================================================================
...
--                  and from _G.ConLogsDungeon_Toggle.
...
local function StartLogging()
    ...
    logEndTime   = nil
    print("|cffffaa44ConLogs|r: |cff66ff66/combatlog started|r for this dungeon run.")
end
...
local function StopLogging()
    ...
    logEndTime = GetTime()
    print("|cffffaa44ConLogs|r: /combatlog stopped.")
end
...
local function OnEnterDungeon(dungeonKey)
    ...
    if instanceType == "raid"
       and ConLogsDB and ConLogsDB.config
       and ConLogsDB.config.raidAutoLog
    then
        ResetRun()
        ...
        if alreadyOn then
            ...
            if addonStartedLog then
                print(string.format("|cffffd200ConLogs|r |cff66ff66RAID AUTO-LOG RESUMED|r |cff888888(%s)|r", dungeonKey))
                ...
                print(string.format("|cffffd200ConLogs|r |cffffd200raid detected|r |cff888888(%s)|r", dungeonKey))
            ...
            print("|cffffaa44=======================================|r")
            print(string.format("|cffffd200ConLogs|r |cff66ff66RAID AUTO-LOG STARTED|r"))
...
local function OnBossKilled(bossName)
    ...
    if def.variants and not currentVariant then
        ...
        if currentVariant then
            ...
            trashKills = {}
            print(string.format("|cffffaa44ConLogs|r: variant auto-detected from boss kill: |cffffd200%s|r",
...
BuildFrame = function()
    local f = CreateFrame("Frame", "ConLogsDungeonFrame", UIParent)
    ...
    f.minimizeBtn:SetScript("OnClick", function(self)
        ...
        ConLogsDB.config.dungeonMinimized = f.minimized
                ...
                for i, vk in ipairs(variantKeys) do
                    ...
                    if not btn then
                        ...
                        btn:SetScript("OnClick", function(self)
                            ...
                            print(string.format("|cffffaa44ConLogs|r: variant selected: |cffffd200%s|r",
    ...
    if ConLogsDB and ConLogsDB.config
       and ConLogsDB.config.dungeonMinimized
    then
        f.minimized = true
...
_G.ConLogsDungeon_Toggle = function()
    if not frame then frame = BuildFrame() end
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_LOGIN" then
        ...
        ConLogsDB = ConLogsDB or {}
        ConLogsDB.config = ConLogsDB.config or {}
        if ConLogsDB.config.raidAutoLog == nil then
        ...
        if frame and ConLogsDB.config.dungeonMinimized ~= nil then
            frame.minimized = ConLogsDB.config.dungeonMinimized
